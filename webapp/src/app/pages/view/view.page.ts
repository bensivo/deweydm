...
export class ViewPageComponent implements OnInit {
    ...
    ngOnInit(): void {
        this.route.params.subscribe(params => {
            ...
            const entity = this.entityService.entities$().find(e => e.id === view.entityId);
            if (!entity) {
                this.router.navigate(['/']);
                return;
            }

            // Set the view name in the header
            this.viewNameSignal.set(view.name);

            // Set the current entity for the filter service
            this.filterService.setCurrentEntity(view.entityId);

            // Clear any existing filters for this entity
